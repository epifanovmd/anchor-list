...
Отступы копии **поперёк оси** обязаны повторять слот объекта в строке:
горизонтальные в вертикальном списке, вертикальные при `horizontal`.

Поперёк оси слой растягивает копию на весь вьюпорт — так же, как список
растягивает строку, — поэтому выравнивание внутри копии считается от тех же
кромок, что и в строке.

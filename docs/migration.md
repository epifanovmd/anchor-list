...
Если список, с которого вы переходите, уже работает с оценкой высоты и типами
строк, модель знакомая — переносится почти один к одному:
...
Что появляется взамен: удержание позиции по двум причинам раздельно, прилипание
на обеих кромках сразу, состояние на UI-потоке и распорки под панель ввода —

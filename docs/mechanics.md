...
Внутри списка — обычный `ScrollView` и ровно четыре ребёнка:
...
Место под панелью ввода отдаётся распоркой в подвале — она часть контента, поэтому
последняя строка честно останавливается над панелью. Но одной распорки мало: она
добавляется в конец, а удержание позиции компенсирует только изменения выше экрана
— видимые строки от неё не двигаются. Чтобы контент поднялся вместе с клавиатурой,
смещение сдвигается явно и на UI-потоке.

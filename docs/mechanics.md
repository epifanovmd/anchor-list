...
**Об оси.** Всё дальше считается вдоль оси скролла и от её направления не
зависит. При [`horizontal`](props.md#horizontal-boolean) «высота» здесь читается
как «ширина», «выше вьюпорта» — как «левее», кромки `start` и `end` — как левая
и правая. Ни одна механика при этом не меняется: расчёт оперирует смещениями и
размерами, а какая это ось, знает только слой отрисовки. Исключение одно — RTL,
см. [Ограничения](limitations.md#особенности-поведения).
...
до конца кадра и идёт одним проходом на всю пачку замеров. Изменение размера
меньше порога считается шумом округления экрана и раскладку не двигает: иначе
каждая доля пикселя запускала бы компенсацию.
...
- **Пачка замеров — один якорь.** Он снимается до первого изменения пачки и
...
- **Скорость.** Компенсация по размерам не снимает якорь, пока список идёт
  быстрее примерно трёх сотых экрана в миллисекунду: на такой скорости рывок не
  виден, а сдвиг стоил бы второго прохода раскладки. Решение принимается в
  момент замера, снятый якорь доводится до конца. Компенсация по данным от
  скорости не зависит.
- **RTL по горизонтали.** Под RTL нативный слой отсчитывает смещение от правой
  кромки, а `MaintainVisibleScrollPositionHelper` на Android считает позицию
  ребёнка как `x + width` в координатах LTR и компенсирует в противоположную
  сторону. Поэтому `horizontal` под RTL не поддерживается.
...
Попытки начинаются только после того, как `ScrollView` отдал замер длины
контента: раньше просить смещение бессмысленно. Смещение просится целыми
точками, а у конца контента округляется вниз: позиции строк дробные, нативный
слой кладёт смещение на пиксель устройства, и без округления цель никогда бы не
«доехала».
...
`contentSize` распорки подставляется текущая. Замер `contentSize` отстаёт на
кадр-другой, и считать от него значило бы поднимать скролл с опозданием.

Детали, из-за которых это работает без дрожания:

- **Одной распорки мало.** Она добавляется в конец, а удержание позиции
  компенсирует только изменения выше вьюпорта. Поэтому смещение список
  поднимает явно, на величину прироста отступа.
- **У самого низа смещение считается от границы контента**, а не сложением
  дельт: накопленная ошибка округления иначе оставляла бы зазор.
- **Распорка идёт на пару кадров впереди отступа** и сходит в ноль, когда
  движение кончилось: место под подъём должно существовать раньше, чем скролл
  туда поедет.
- **Жест приоритетнее.** Пока палец на экране или идёт инерция, позицией
  распоряжается жест, и подъём не применяется.
- **До первого показа смещение не трогается**, но отступ учитывается к моменту
  показа, чтобы список открылся сразу над панелью.

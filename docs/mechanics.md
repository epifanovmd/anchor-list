...
Механика, ради которой я и писал список. Разберём на числах.
...
глазах. Готовым считается фактическое окно рассчитанного offset вместе с
`drawDistance`: при нижнем выравнивании это строки перед целью, а не после неё.
Вдобавок каждая попытка сверяет живое нативное смещение с тем, что просила
прошлая: команду перебивает нативная компенсация замеров и обрезает граница
контента, а показанную позицию уже не поправить.
Если размеры так и не устаканились, показ завершает конечная страховка по
времени — считать кадры нельзя, нативный commit может отставать от них.

Тот же механизм отвечает за первый показ вообще: список раскрывается, когда
видимые строки измерены — или когда их размеры объявлены пропом и измерять
нечего. На случай, когда измерений не приходит вовсе (нулевая высота ячеек),
есть страховка по времени. Во время доводки начальной позиции относительная
компенсация и автоприлипание к концу не запускаются: абсолютным offset управляет

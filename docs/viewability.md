...
**Доля строки на UI-потоке.** Пороги отвечают «да/нет» в JS. Непрерывную долю
строки во вьюпорте — для параллакса и затухания у кромок — отдаёт
[`useAnchorListItemVisibility`](state.md#видимость-строки-на-ui-потоке) внутри
ячейки; она учитывает `insetEnd` и сдвиг прилипших якорей, но не знает
выдержки `minimumViewTime`.

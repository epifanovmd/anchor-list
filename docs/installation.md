...
Бандл примера собирается — но это единственное, что про web известно, и
проверялось оно вручную. В браузере список не запускался, CI web не трогает, и
рассчитывать на него сейчас нельзя.
...
`react-native-web` этого свойства не реализует. То есть механика, ради которой
список и написан, на web не работает вовсе, а не «ведёт себя иначе».

Поддержка web в планах есть, но потребует отдельного пути компенсации позиции.
Отслеживать — в [issues](https://github.com/epifanovmd/anchor-list/issues).
...
экспортируются намеренно: они меняются вместе с реализацией, и опираться на них
нельзя.

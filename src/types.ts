...
export interface IAnchorListProps<TItem> {
  ...
  /**
   * Зазор между строками, px.
   *
   * Лежит **между** слотами, а не внутри них — как `gap` во flexbox: перед
   * первой строкой и после последней его нет. Строка для списка — это ровно
   * то, что вернул `renderItem`: размер, видимость, прилипание и подсветка
   * считаются по ней, а не по ней плюс отступ. Зазор в `margin` корня строки
   * так не работает: измеряется узел, а не его внешние отступы.
   *
   * @default 0
   */
  gap?: number;
   ...
   *
   * Для переписки: между сообщениями одного автора 4, между группами 12.

...
export const useInsetEnd = ({
  ...
  useAnimatedReaction(
    ...
    ({ inset, base, length }) => {
      ...
      // Отметка ставится сразу, а не вместе с первым движением: кадры сюда
      // приходят и от роста элементов, и если первый из них ушёл ни с чем —
      // отступ не менялся, — то следующий сравнивал бы с нулём и получал
      // дельту во весь отступ. Список при этом стоит наверху, и «подъём» на
      // эту дельту уводит первую строку за верхнюю кромку.
      if (!seeded.value) {
        seeded.value = true;

...
describe("resolveInsetEnd", () => {
  ...
  it("снимает запас, когда отступ встал", () => {
    // Шаг отступа фиксируется как скорость его хода, но следующего кадра
    // движения может и не быть: так меняется отступ от замеренной панели ввода
    // или от смены безопасной зоны. Дальше кадры приходят от роста элементов, и
    // запас, взятый по тому шагу, остаётся в контенте до ближайшего движения —
    // у конца списка это пустая полоса под последней строкой.
    const { spacer } = resolve({
      rows: 5000,
      previousInset: OPEN,
      previousDelta: 52,

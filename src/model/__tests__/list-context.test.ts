...
import type {
  IAnchorListContextValue,
  IAnchorListRuntimeHandle,
} from "../list-context";
import {
  ListContextProvider,
  useListRuntime,
...
const createValue = (): IAnchorListContextValue => ({
  store: new ListStore(),
  runtime: {
    getItemAt: () => undefined,
  ...
  layout: {
    scrollLength: { value: 0 } as SharedValue<number>,
    contentOrigin: { value: 0 } as SharedValue<number>,
    insetEnd: undefined,
    alignOffset: { value: 0 } as SharedValue<number>,
  },
  horizontal: false,
  stickyPinned: {

...
Список одноколоночный. Ось задаётся пропом `horizontal`; под RTL горизонтальный
режим не поддерживается — см. [Ограничения](docs/limitations.md).
...
- [x] Горизонтальная ось (`horizontal`) — все механики те же, кроме RTL
...
- [ ] Горизонтальная ось под RTL — упирается в нативное удержание позиции
